...
from evaluation_fn import threat_score
...
class RoadMap(pl.LightningModule):
    ...
    def threat_score(self, outputs, y):
    ...
    def _run_step(self, batch, batch_idx):
        ...
        y = torch.stack(road_image, dim=0).float()

        # forward pass to calculate outputs
        outputs = self(x)

        # to calculate confusion matrix we have to go from scores to 0/1 predictions
        # NOTE this is using 0.5 threshold (!!!)
        # preds = outputs.round()

        # calculate threatscore
        # first, calculate confusion matrix
        # ---add---
